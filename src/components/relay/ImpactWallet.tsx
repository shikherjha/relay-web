...
import { getImpact, walletEarlyAccess, type ImpactWalletDTO } from "@/lib/relay-api";
        ...
        <button className="flex items-center gap-3 rounded-full border border-border bg-card px-3 py-1.5 text-sm hover:shadow-sm transition-shadow">
          <span className="flex items-center gap-1.5">
            ...
            <span className="tabular font-medium">
              <AnimatedCounter value={co2Kg} decimals={1} /> kg
            </span>
          </span>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-xl bg-secondary p-3">
              ...
              <div
                className="text-2xl font-display tabular mt-1"
          ...
          >
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              {walletEarlyAccess(wallet)
                ? `${wallet.tier ?? "Early"}-tier access — you see new rescue listings before everyone else.`
                : `Earn ${Math.max(0, wallet.credits_to_next_tier ?? 0).toFixed(0)} more credits to unlock tiered early access to the rescue feed.`}
            </p>
          </div>
          <Link
            to="/impact"
